...
This folder contains the real co-located low-cost-sensor + reference
dataset used for the secondary, real-data validation case study in the
manuscript (Section 5.6). It addresses Reviewer 1's concern that the
synthetic PMS5003 error model used in the primary KAPSARC case might
limit the real-world applicability of the uncertainty evaluation.
...
1. Reads only the **Hawthorne (HW)** sheet — the only sheet that contains
   environmental covariates (T, RH, wind), FEM PM₂.₅, FEM PM₁₀, and three
   co-located PMS5003 nodes.
...
6. Adds `day` (days since first sample) and `hour_of_day` features so the
   schema matches the KAPSARC cleaned data.
...
If you use this dataset in your own work, please cite **both** the data
repository and the Kelly et al. AMT paper:
